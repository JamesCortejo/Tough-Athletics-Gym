...
class PDFGenerator {
  ...
  safeDecryptText(value, fallback) {
    if (value === undefined || value === null || value === "") {
      return fallback;
    }

    const raw = String(value);
    let decrypted = encryptionService.decrypt(raw);

    if (encryptionService.isEncrypted(decrypted)) {
      decrypted = encryptionService.decrypt(String(decrypted));
    }
  ...
  validateMembershipData(membership) {
    return {
      ...
      email: this.safeDecryptText(membership.email, "No email"),
    };
  }
  ...
  validateNonMemberData(nonMember) {
    return {
      ...
      phone: this.safeDecryptText(nonMember.phone, "No phone"),
      email: this.safeDecryptText(nonMember.email, "No email"),
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (Array.isArray(users)) {
          users.forEach((user) => {
            if (user && user._id) {
              userEmailById.set(user._id.toString(), user.email);
            }
          });
        }
        ...
        const validatedMemberships = filtered.map((m) => {
          const userId = m?.userId ? m.userId.toString() : null;
          const userEmail = userId ? userEmailById.get(userId) : null;
          return this.validateMembershipData({
            ...m,
